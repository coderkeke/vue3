      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'action'">
            <a-popconfirm title="确定要删除这个停用词吗？" @confirm="handleDelete(record.id)">
...
const fetchCategoryOptions = async () => {
  try {
    const res = await getStopWordsCategories()
    const result = res.data
    if (result.success) {
      categoryOptions.value = result.data?.map((i) => ({ label: i, value: i })) || []
    }
  } catch (error) {
    console.error('获取停用词分类失败:', error)
  }
}
...
const addSchemas = computed<FormSchema[]>(() => [
  {
    ...
    componentProps: {
      allowClear: true,
      rows: 4,
      placeholder: '请输入停用词，多个词用英文逗号分隔。例如：的,了,和,是',
    },
    rules: [{ required: true, message: '请输入停用词' }],
    colProps: { span: 24 },
  },
  {
    field: 'category',
...
const fetchTableData = async (page = 1, pageSize = 10) => {
  ...
  try {
    const res = await getStopWordsPage({
      page: page,
      size: pageSize,
    ...
    // res is UnifiedResponse<StopWordPageResult>
    const result = res.data
    ...
    // message.error('获取列表失败') // HttpService handles error message
  } finally {
...
const handleAdd = async () => {
  ...
  try {
    const res = await addStopWordsBatch({
      words: addForm.words,
